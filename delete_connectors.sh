...
# Exibir conectores
echo "Lista de conectores disponíveis para REMOVER:"
...
# Entrada do usuário
echo ""
read -p "Digite os números dos conectores que deseja remover (ex: 2 5 8), ou 'all' para todos: " SELECAO
...
for ID in "${IDS[@]}"; do
  ...
  echo "🗑️  Removendo conector: $NAME"
  RESPONSE=$(curl -s -w "HTTPSTATUS:%{http_code}" -X DELETE "$KAFKA_CONNECT_URL/$NAME")

  BODY=$(echo "$RESPONSE" | sed -e 's/HTTPSTATUS\:.*//g')
  STATUS=$(echo "$RESPONSE" | tr -d '\n' | sed -e 's/.*HTTPSTATUS://')

  if [[ "$STATUS" -ge 200 && "$STATUS" -lt 300 ]]; then
    echo "✅ Conector $NAME removido com sucesso!"
  else
    echo "❌ Erro ao remover $NAME (HTTP $STATUS)"
    echo "📄 Detalhes:"
    echo "$BODY"
  fi
