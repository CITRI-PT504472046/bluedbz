...
echo "Lista de conectores disponíveis para INICIAR:"
...
# Seleção interativa
echo ""
read -p "Digite os números dos conectores que deseja iniciar (ex: 2 5 7), ou 'all' para todos: " SELECAO
...
for ID in "${IDS[@]}"; do
  ...
  echo "⏸️  Iniciando conector: $NAME"
  RESPONSE=$(curl -s -w "HTTPSTATUS:%{http_code}" \
    -X PUT "$KAFKA_CONNECT_URL/$NAME/resume")

  BODY=$(echo "$RESPONSE" | sed -e 's/HTTPSTATUS\:.*//g')
  STATUS=$(echo "$RESPONSE" | tr -d '\n' | sed -e 's/.*HTTPSTATUS://')

  if [[ "$STATUS" -ge 200 && "$STATUS" -lt 300 ]]; then
    echo "✅ Conector $NAME iniciado com sucesso!"
  else
    echo "❌ Erro ao iniciar $NAME (HTTP $STATUS)"
    echo "📄 Detalhes:"
    echo "$BODY"
  fi
